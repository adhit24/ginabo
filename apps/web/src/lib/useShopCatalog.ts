...
export const STATIC_BUNDLES: ShopProduct[] = [
  {
    id: "ginabo-complete-skin", slug: "ginabo-complete-skin", name: "Ginabo Complete Skin Nutrition Set",
  ...
  {
    id: "repair-glow-set", slug: "repair-glow-set", name: "Repair & Glow Set",
  ...
  {
    id: "daily-barrier-routine-set", slug: "daily-barrier-routine-set", name: "Daily Skin Barrier Set",
  ...
  {
    id: "bright-renewal-set", slug: "bright-renewal-set", name: "Bright Renewal Set",
    ...
    listActiveProducts()
      .then((products) => {
        ...
        const storeBundles = store.getBundles().map(p => ({
          id: p.id, slug: p.slug || p.id, name: p.name, category: "bundling",
