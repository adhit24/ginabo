...
const HOW_TO_USE_STEPS = [
  { step: 1, title: "Bersihkan Wajah", desc: "Bersihkan wajah terlebih dahulu dengan facial wash atau cleanser lembut, lalu keringkan perlahan dengan handuk bersih." },
  { step: 2, title: "Aplikasikan Produk", desc: "Keluarkan 2–3 tetes atau secukupnya ke telapak tangan yang bersih atau langsung ke area dahi dan pipi." },
  { step: 3, title: "Ratakan & Pijat Lembut", desc: "Ratakan ke seluruh wajah dan leher dengan gerakan memijat ke arah atas hingga formula meresap sempurna." },
...
const ACTIVE_INGREDIENTS = [
  { name: "Niacinamide (5%)", desc: "Mencerahkan noda hitam, meratakan warna kulit, dan menyamarkan tampilan pori-pori." },
  { name: "Alpha Arbutin", desc: "Membantu menekan produksi melanin berlebih untuk kulit tampak cerah bercahaya." },
  { name: "Ceramide Complex", desc: "Mengunci kelembapan alami dan memperkuat lapisan pelindung skin barrier." },
  { name: "Salmon DNA / Peptide", desc: "Meremajakan sel kulit, menjaga elastisitas, dan mempercepat regenerasi sel." },
  { name: "Hyaluronic Acid", desc: "Memberikan hidrasi mendalam hingga ke lapisan kulit terdalam agar kenyal." },
];
...
const FAQS = [
  { q: "Apakah produk ini cocok untuk kulit sensitif?", a: "Ya, seluruh produk Ginabo diformulasikan secara dermatologis (hypoallergenic & non-comedogenic) sehingga aman dan nyaman digunakan untuk semua jenis kulit, termasuk kulit sensitif dan mudah berjerawat." },
  ...
  { q: "Kapan waktu terbaik menggunakannya?", a: "Dapat digunakan setiap hari pada rutinitas pagi (AM) sebelum sunscreen dan malam (PM) sebelum tidur untuk hasil maksimal yang optimal." },
...
export function ProductDetailClient({ product }: Props) {
  ...
  return (
    <div className="min-h-screen bg-[#FFFFFF] font-sans antialiased text-[#292929]">
      {/* ── Top Announcement / Breadcrumb Bar ── */}
      <div className="border-b border-[#EAEAEA] bg-white py-3.5 px-4 md:px-8">
        <div className="mx-auto max-w-7xl">
          <nav className="flex items-center flex-wrap gap-2 text-[13px] text-[#7C7C7C] font-medium">
            ...
            <span className="text-[#C0C0C0]">/</span>
            <span className="text-[#292929] font-semibold truncate max-w-[240px] md:max-w-none">{product.name}</span>
          </nav>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-14 items-start">
          ...
          <div className="lg:col-span-5 flex flex-col gap-4 sticky top-24">
            ...
            <div 
              ...
              className="relative aspect-square w-full rounded-[14px] bg-[#FAF8FC] border border-[#E5E5E5] shadow-xs overflow-hidden cursor-crosshair select-none group"
            >
              ...
              >
                <span className="inline-flex items-center gap-1.5 rounded-full bg-black/60 backdrop-blur-md px-3.5 py-1 text-[12px] font-semibold text-white shadow-sm">
              ...
              {gallery.length > 1 && (
                <>
                  <button
                    ...
                    className="absolute left-3 top-1/2 -translate-y-1/2 flex h-10 w-10 items-center justify-center rounded-full bg-white/95 shadow-md text-[#292929] hover:bg-white hover:text-[#78257C] transition-all opacity-85 hover:opacity-100 focus:outline-none"
                  >
                  ...
                  <button
                    ...
                    className="absolute right-3 top-1/2 -translate-y-1/2 flex h-10 w-10 items-center justify-center rounded-full bg-white/95 shadow-md text-[#292929] hover:bg-white hover:text-[#78257C] transition-all opacity-85 hover:opacity-100 focus:outline-none"
                  >
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              ...
              <div className="absolute bottom-3 inset-x-0 flex justify-center items-center pointer-events-none">
                <div className="flex items-center gap-2 rounded-full bg-white/95 backdrop-blur-md px-4 py-1 text-[13px] font-bold text-[#292929] shadow-sm border border-[#EBEBEB]">
            ...
            <div className="relative">
              ...
              <button
                ...
                className="absolute -left-2 top-1/2 -translate-y-1/2 z-10 hidden md:flex h-8 w-8 items-center justify-center rounded-full bg-white shadow-md border border-gray-200 text-gray-700 hover:text-[#78257C] hover:scale-105 transition"
              >
                ...
                {gallery.map((card, idx) => {
                  ...
                  return (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.95 }}
                      className={`relative flex-shrink-0 w-[80px] md:w-[90px] h-[98px] md:h-[108px] rounded-[10px] bg-white border transition-all flex flex-col items-center justify-between p-1.5 snap-start text-left overflow-hidden ${
                        isActive
                          ? "border-[#78257C] ring-2 ring-[#78257C]/25 shadow-md"
                          : "border-[#E5E5E5] hover:border-gray-300 opacity-80 hover:opacity-100"
                      }`}
                    >
                      {/* Badge if present */}
                      {card.badge && (
                        <span className="absolute top-1 left-1 z-10 rounded-[3px] bg-[#78257C] px-1 py-0.2 text-[9px] font-extrabold text-white uppercase tracking-tight">
                          {card.badge}
                        </span>
                      )}
                      ...
                      <div className="w-full bg-[#F5F5F5] rounded-[4px] py-1 px-1 text-center">
                        <p className="truncate text-[10px] font-bold text-[#303030]">
                          {card.label}
                        </p>
                      </div>
                      ...
                      {isActive && (
                        <motion.span
                          ...
                          className="absolute inset-x-0 bottom-0 h-[3px] bg-[#78257C]"
              ...
              <button
                ...
                className="absolute -right-2 top-1/2 -translate-y-1/2 z-10 hidden md:flex h-8 w-8 items-center justify-center rounded-full bg-white shadow-md border border-gray-200 text-gray-700 hover:text-[#78257C] hover:scale-105 transition"
              >
          ...
          <div className="lg:col-span-7 flex flex-col gap-5">
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex flex-col gap-1.5">
                <h1 className="text-[24px] md:text-[30px] lg:text-[32px] font-extrabold text-[#292929] leading-tight tracking-tight">
                  {product.name}
                </h1>
                <p className="text-[14px] md:text-[15.5px] text-[#7C7C7C] font-medium leading-normal">
                  {subtitle}
                </p>
              </div>
              ...
              <div className="relative flex-shrink-0" ref={shareRef}>
                <button
                  ...
                  className="flex items-center gap-2 text-[13.5px] font-bold text-[#555555] hover:text-[#78257C] px-3.5 py-1.5 rounded-full border border-[#D5D5D5] hover:border-[#78257C] transition bg-white shadow-xs"
                >
                  <span>Share</span>
                  ...
                  {shareOpen && (
                    ...
                    >
                      <button
                        ...
                        className="flex w-full items-center gap-2.5 rounded-[6px] px-3 py-2 text-[13px] font-medium text-[#292929] hover:bg-[#F6EDF8] hover:text-[#78257C] transition"
                      >
                        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M10 13a5 5 0 0 0 7.5.5l2-2a5 5 0 0 0-7-7l-1 1" /><path d="M14 11a5 5 0 0 0-7.5-.5l-2 2a5 5 0 0 0 7 7l1-1" /></svg>
            ...
            <div className="flex items-center flex-wrap gap-3.5 pt-1">
              <span className="text-[28px] md:text-[34px] font-extrabold text-[#78257C] tracking-tight">
                {formatPrice(product.priceMinor)}
              </span>
              <span className="text-[15px] md:text-[17px] text-[#A0A0A0] line-through font-normal">
                {formatPrice(originalPriceMinor)}
              </span>
              <span className="rounded-[4px] bg-[#FCE7F3] text-[#DB2777] px-2.5 py-0.5 text-[13px] font-extrabold">
                -{discountPercent}%
              </span>
            </div>
            ...
            <div className="flex flex-col gap-2 pt-1">
              <span className="text-[13px] font-bold text-[#525252]">Quantity</span>
              <div className="flex items-center">
                <div className="inline-flex items-center rounded-full border border-[#D5D5D5] bg-white px-1.5 py-1 shadow-xs">
                  <button
                    onClick={() => setQty((q) => Math.max(1, q - 1))}
                    disabled={qty <= 1}
                    className="flex h-8 w-8 items-center justify-center rounded-full text-[17px] font-semibold text-[#404040] hover:bg-[#F3F3F3] active:scale-90 transition disabled:opacity-30 disabled:cursor-not-allowed"
                    aria-label="Kurangi jumlah"
                  >
                    −
                  </button>
                  <span className="w-11 text-center text-[15px] font-extrabold text-[#292929] select-none">
                    {qty}
                  </span>
                  <button
                    onClick={() => setQty((q) => q + 1)}
                    className="flex h-8 w-8 items-center justify-center rounded-full text-[17px] font-semibold text-[#404040] hover:bg-[#F3F3F3] active:scale-90 transition"
            ...
            <div className="pt-2">
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className={`relative w-full flex items-center justify-center gap-2.5 rounded-[8px] py-4 text-[15px] md:text-[16px] font-extrabold text-white shadow-md transition-all tracking-wide ${
                  added
                    ? "bg-emerald-600 shadow-emerald-200"
                    : product.stockQty === 0
                    ? "bg-gray-400 cursor-not-allowed"
                    : "bg-[#78257C] hover:bg-[#681f6c] shadow-[#78257C]/25"
                }`}
              >
                {added ? (
                  <>
            ...
            <div className="mt-4 pt-4 border-t border-[#EAEAEA] flex flex-col gap-4">
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className={`relative rounded-full px-6 py-2.5 text-[13px] md:text-[14px] font-extrabold uppercase tracking-wider transition-all ${
                    activeTab === "details"
                      ? "bg-[#78257C]/10 text-[#78257C] border border-[#78257C]/30 shadow-xs"
                      : "bg-transparent text-[#707070] hover:text-[#292929] border border-transparent"
                  }`}
                >
                  DETAILS
                </button>
                ...
                <AnimatePresence mode="wait">
                  {activeTab === "details" ? (
                    <motion.div
                      ...
                      className="flex flex-col gap-3.5 text-[14.5px] leading-relaxed text-[#4A4A4A]"
                    >
                      {/* Product Description */}
                      <p>
                        {product.description ||
                      ...
                      <div className="flex flex-col gap-2 pt-1">
                        {CERTIFICATIONS.map((cert, i) => (
                          <div key={i} className="flex items-center gap-2.5 text-[#292929] font-medium text-[13.5px]">
                            <span className="flex h-5 w-5 items-center justify-center rounded-[4px] bg-emerald-50 text-emerald-600">
                      ...
                      {product.weightGrams !== null && (
                        <p className="text-[13px] text-[#707070] pt-1">
                          Netto / Berat: <span className="font-bold text-[#292929]">{product.weightGrams} gram</span>
                        </p>
                      )}
                    </motion.div>
                  ) : (
                    ...
                    >
                      <p className="text-[13.5px] text-[#555555] font-semibold">
                        Langkah Penggunaan Rutin (AM / PM):
                      </p>
                      <div className="flex flex-col gap-3.5">
                        {HOW_TO_USE_STEPS.map((item) => (
                          <div key={item.step} className="flex items-start gap-3.5 text-[14px]">
                            <span className="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-[#78257C] text-[12px] font-bold text-white">
                              {item.step}
                            </span>
                            <div className="flex flex-col">
                              <span className="font-bold text-[#292929] text-[14px]">{item.title}</span>
                              <span className="text-[#555555] text-[13.5px] leading-relaxed">{item.desc}</span>
                            </div>
                          </div>
                        ))}
                      </div>
            ...
            <div className="border-t border-[#EAEAEA] flex flex-col divide-y divide-[#EAEAEA]">
              ...
              <div className="py-3.5">
                <button
                  ...
                  className="flex w-full items-center justify-between py-1 text-left text-[14px] font-bold text-[#292929] hover:text-[#78257C] transition tracking-wide"
                >
                ...
                <AnimatePresence>
                  {activeAccordion === "ingredients" && (
                    <motion.div
                      ...
                      transition={{ duration: 0.25, ease: EASE }}
                      className="overflow-hidden pt-3.5 flex flex-col gap-3.5"
                    >
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {ACTIVE_INGREDIENTS.map((ing, i) => (
                          <div key={i} className="rounded-[8px] bg-[#FAF8FC] border border-[#EAE5F5] p-3">
                            <p className="text-[13px] font-bold text-[#78257C]">{ing.name}</p>
                            <p className="text-[12px] text-[#555555] leading-snug">{ing.desc}</p>
                          </div>
                        ))}
                      </div>
                      <p className="text-[12px] text-[#7C7C7C] leading-relaxed pt-1">
                        <span className="font-bold text-[#444444]">Full Ingredients:</span> {FULL_INGREDIENTS}
                      </p>
                    </motion.div>
                  )}
                </AnimatePresence>
                ...
                <AnimatePresence>
                  {activeAccordion === "faq" && (
                    <motion.div
                      ...
                      exit={{ height: 0, opacity: 0 }}
                      transition={{ duration: 0.25, ease: EASE }}
                      className="overflow-hidden pt-3.5 flex flex-col gap-3"
                    >
                      {FAQS.map((faq, i) => (
                        <div key={i} className="rounded-[8px] bg-white border border-[#EAEAEA] p-3.5 text-[13.5px]">
                          <p className="font-bold text-[#292929] mb-1">{faq.q}</p>
        ...
        <section className="border-t border-[#EAEAEA] bg-white py-12 md:py-16">
          <div className="mx-auto max-w-7xl px-4 md:px-8">
            <div className="text-center mb-8">
              <h2 className="text-[20px] md:text-[24px] font-extrabold uppercase tracking-widest text-[#292929]">
                YOU MAY ALSO LIKE
              </h2>
              <p className="text-[14px] text-[#707070] mt-1">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
              {related.slice(0, 4).map((item) => (
                <div
                  key={item.slug}
                  className="group flex flex-col rounded-[12px] border border-[#EBEBEB] bg-white p-3.5 shadow-xs hover:shadow-md transition-all hover:border-[#78257C]/40 relative"
                >
                  {/* Discount badge */}
                  <span className="absolute top-3 right-3 z-10 rounded-[4px] bg-[#FCE7F3] text-[#DB2777] px-2 py-0.5 text-[11px] font-bold">
                    -30%
                  </span>

                  {/* Best Seller Tag */}
                  <span className="absolute top-3 left-3 z-10 rounded-[3px] bg-[#F5F5F5] border border-[#E0E0E0] text-[#555555] px-1.5 py-0.5 text-[10px] font-semibold">
                    Best Seller
                  </span>
                  ...
                  <div className="flex flex-col flex-1 gap-1">
                    <Link href={`/shop/${item.slug}`} className="line-clamp-2 text-[15px] font-bold text-[#292929] group-hover:text-[#78257C] transition leading-snug">
                      {item.name}
                    </Link>
                    <div className="flex items-baseline gap-2 mt-auto pt-2">
                      <span className="text-[16px] font-extrabold text-[#78257C]">
                        {item.price}
                      </span>
                      {item.originalPrice && (
                        <span className="text-[12.5px] text-[#A0A0A0] line-through">
                          {item.originalPrice}
                        </span>
                      )}
                    </div>
                  ...
                  <button
                    ...
                    className="mt-3 w-full flex items-center justify-center gap-1.5 rounded-[6px] bg-[#78257C] py-2.5 text-[13px] font-bold text-white hover:bg-[#681f6c] transition active:scale-95 shadow-xs"
                  >
        ...
        <div className="mx-auto max-w-7xl px-4 md:px-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center md:text-left">
            ...
            <Link href="/stores" className="flex items-center gap-4 p-4 rounded-[10px] bg-white border border-[#EBEBEB] hover:border-[#78257C]/40 transition group">
              <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-[#78257C]/10 text-[#78257C] group-hover:scale-110 transition">
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z" /><circle cx="12" cy="10" r="3" />
                </svg>
              </div>
              <div>
                <p className="text-[14px] font-bold text-[#292929] uppercase tracking-wide">STORE LOCATOR</p>
                <p className="text-[12.5px] text-[#707070]">Temukan toko resmi online &amp; offline terdekat</p>
              </div>
            </Link>
            ...
            <Link href="/reseller" className="flex items-center gap-4 p-4 rounded-[10px] bg-white border border-[#EBEBEB] hover:border-[#78257C]/40 transition group">
              ...
              <div>
                <p className="text-[14px] font-bold text-[#292929] uppercase tracking-wide">BECOME A RESELLER</p>
                <p className="text-[12.5px] text-[#707070]">Dapatkan komisi tinggi &amp; reward eksklusif</p>
              </div>
            </Link>
            ...
            <Link href="/faq" className="flex items-center gap-4 p-4 rounded-[10px] bg-white border border-[#EBEBEB] hover:border-[#78257C]/40 transition group">
              ...
              <div>
                <p className="text-[14px] font-bold text-[#292929] uppercase tracking-wide">FAQ &amp; BANTUAN</p>
                <p className="text-[12.5px] text-[#707070]">Panduan lengkap seputar pesanan &amp; produk</p>
              </div>
            </Link>
          </div>
        </div>
